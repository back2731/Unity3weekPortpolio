...
public class SpawnBeamLaser : MonoBehaviour
{
    [Header("Prefabs")]
    ...
    public float beamLength = 125.0f; //Ingame beam length
    public float beamEndOffset = 0f; //How far from the raycast hit point the end effect is positioned
    public float textureScrollSpeed = 15.0f; //How fast the texture scrolls along the beam, can be negative or positive.
    public float textureLengthScale = 5.0f;   //Set this to the horizontal length of your texture relative to the vertical. 
    ...
    void FixedUpdate()
    {
        if (GameManager.instance.isGame)
        {
            if (beam) //Updates the beam
            {
                ...
                int layerMask = 1 << LayerMask.NameToLayer("ENEMY") | LayerMask.NameToLayer("WALL");
